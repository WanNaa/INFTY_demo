...
from utils.feedback_degradation import FeedbackDegradationController
from utils.efficiency import EfficiencyTracker
...
class Learner(BaseLearner):
    def __init__(self, args):
        ...
        self.feedback_controller = FeedbackDegradationController(args)
        self.efficiency_tracker = EfficiencyTracker(self._device, enabled=args.get("efficiency_logging_enabled", False))
    ...
    def incremental_train(self, data_manager):
        self._cur_task += 1
        self.feedback_controller.reset_task(self._cur_task)
    ...
    def create_loss_fn(self, inputs, targets, model=None, feedback_context=None):
        """
        ...
        if feedback_context is None:
            feedback_context = {
                ...
                "sample_mask": torch.ones(targets.shape[0], device=targets.device, dtype=torch.bool),
                "label_mask": torch.ones(targets.shape[0], device=targets.device, dtype=torch.bool),
            }
        def loss_fn():
            logits = model(inputs, test=False)["logits"]
            per_sample_loss = F.cross_entropy(logits, targets, reduction="none")
            loss = self.feedback_controller.apply_to_per_sample_loss(per_sample_loss, feedback_context)
            return logits, [loss]
        return loss_fn
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        self.efficiency_tracker.start_task(self._cur_task, optimizer, epochs)
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                self.efficiency_tracker.start_iteration(epoch, i)
                ...
                if self.args["inftyopt"] == "forward_grad":  
                    ...
                    feedback_context = self.feedback_controller.begin_batch(aux_targets.shape[0], self._device)
                    loss_fn = self.create_loss_fn(inputs, aux_targets, feedback_context=feedback_context)
                    optimizer.set_closure(loss_fn)
                logits, loss_list = optimizer.step()
                losses += sum(loss_list)
                self.efficiency_tracker.end_iteration(loss_list)
        ...
        logging.info(info)
        task_efficiency_record = self.efficiency_tracker.end_task()
        if task_efficiency_record is not None:
            task_efficiency_record.update(
                {
            ...
            self._efficiency_task_records.append(task_efficiency_record)
            logging.info(
                "[Efficiency] task=%s mean_iter_ms=%.3f total_time_s=%.3f peak_mem_mb=%s backward_graph_avoided=%s",
                self._cur_task,
                task_efficiency_record["mean_iteration_time_ms"],
                task_efficiency_record["task_train_time_s"],
                "NA" if task_efficiency_record["peak_gpu_memory_mb"] is None else f"{task_efficiency_record['peak_gpu_memory_mb']:.2f}",
                task_efficiency_record["backward_graph_avoided"],
            )
