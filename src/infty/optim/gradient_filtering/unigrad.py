...
class UniGrad(EasyCLMultiObjOptimizer):
    ...
    def step(self, closure=None, delay=False):
        ...
        if self.task_id == 0:
            ...
            if len(loss_list) != 2:
                raise ValueError("UniGrad only supports two losses: old_loss and new_loss")
            ...
            for k in range(len(self.k_idx)):
                ...
                if s_t < current_s_t:
                    w1 = norm_g1 * (
                        current_s_t * torch.sqrt(1 - s_t**2) - s_t * torch.sqrt(1 - current_s_t**2)
                    ) / (norm_g2 * torch.sqrt(1 - current_s_t**2) + 1e-8)
                    w2 = norm_g2 * (
                        current_s_t * torch.sqrt(1 - s_t**2) - s_t * torch.sqrt(1 - current_s_t**2)
                    ) / (norm_g1 * torch.sqrt(1 - current_s_t**2) + 1e-8)
                    g1_new = g1 + g2 * w1
                    g2_new = g2 + g1 * w2
                    uni_grads[0, beg:end] = g1_new
                    uni_grads[1, beg:end] = g2_new
                    self.S_T[k] = (1 - self.beta) * current_s_t + self.beta * s_t
                self._record_conflict_pair(
                    g1,
                    g2,
                    block=k,
                    threshold=current_s_t,
                    task=self.task_id,
                    iteration=iteration,
                    g_old_after=g1_new,
                    g_new_after=g2_new,
    ...
    def __repr__(self):
        return f"UniGrad({self.base_optimizer.__class__.__name__})"
