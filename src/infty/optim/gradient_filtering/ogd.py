...
class OGD(EasyCLMultiObjOptimizer):
    def __init__(self, params, base_optimizer, model, args, **kwargs):
        ...
        if basis_path is None:
            ckp_root = Path(getattr(args_ns, "ckp_dir", REPO_ROOT / "workdirs" / "checkpoints")).expanduser()
    ...
    def _update_mem(self, train_loader, task_count):
        ...
        if self.strategy == "base":
            ogd_train_loader = torch.utils.data.DataLoader(self.task_memory[task_count], batch_size=1, shuffle=False)
            ...
            ogd_train_loader = torch.utils.data.DataLoader(ogd_memory, batch_size=1, shuffle=False)
        else:
            raise ValueError(f"Unsupported OGD strategy: {self.strategy}")
        ...
        print(
            f"[OGD] Updated basis: shape={self.ogd_basis.shape}, "
    ...
    def get_backbone_indices(self):
        end_idx = 0
        for name, param in self.model.named_parameters():
            if param.requires_grad and param.grad is not None and "backbone" in name:
                end_idx += param.numel()
        return end_idx
