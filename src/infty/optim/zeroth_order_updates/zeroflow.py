...
class ZeroFlow(Optimizer):
    ...
    def get_execution_path_metadata(self):
        if self.inftyopt == "forward_grad":
            return {
                "backward_graph_avoided": True,
                "verification_mode": "code_path",
                "execution_path": "ZeroFlow.forward_grad_step -> torch.func.jvp -> parameter update without loss.backward",
                "graph_context": "forward_mode_jvp",
            }
        return {
            "backward_graph_avoided": True,
            "verification_mode": "code_path",
            "execution_path": "ZeroFlow.set_closure -> torch.inference_mode scalar feedback -> finite_difference_estimator -> base_optimizer.step",
            "graph_context": "torch.inference_mode_no_backward",
        }
