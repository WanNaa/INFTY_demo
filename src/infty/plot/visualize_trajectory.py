...
def plot_contour(F, init, traj, trainer, output_dir=None, save_path=None, plotbar=False, grid_size=500):
...
def cagrad(x, optimizer, c=0.5):
    if minimize_scalar is None:
        raise ImportError("scipy is required for cagrad trajectory visualization")
    f, grads = F(x, True)
    g1 = grads[:,0]
    g2 = grads[:,1]
    g0 = (g1+g2)/2
...
def visualize_trajectory(optimizer_name, init=None, n_iter=10000, lr=0.1, output_dir=None, grid_size=500):
    if init is None:
        init = [-8.5, 5.0]

    assert optimizer_name in maps.keys()
    traj = run(optimizer_name, lr, init, n_iter)
    plot_contour(F, init, traj, optimizer_name, output_dir=output_dir, grid_size=grid_size)
    return traj
